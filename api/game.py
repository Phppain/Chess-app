class Board:
    ...
    def is_check(self, color):
        ...
        for piece in self.get_all_pieces(opponent_color=color):
    ...
    def serialize(self):
        ...
        for position, piece in self._board_position.items():
            result[str(position)] = {
                "type": piece.__class__.__name__ if piece else None,
                "color": piece.color if piece else None,
                "position": piece.position if piece else None,
            }
        return result
    ...
    def deserialize(self, data):
        ...
        for pos_str, piece_data in data.items():
            position = eval(pos_str)
            if piece_data["type"]:
                piece_class = globals()[piece_data["type"]]
                piece = piece_class(piece_data["color"], position, self)
                self._board_position[position] = piece
            else:
                self._board_position[position] = None
...
class Piece:
    ...
    def can_move(self, new_position):
        print('CAN_MOVE_BOARD:', self.board)
        board = self.board.serialize()
        
        try:
            
            # print(type(new_position), new_position)
            piece = board[str(new_position)]
        except KeyError:
            print('RETURN none')
            return None
        print('Piece:', piece, new_position)

        print('P', self.board._board_position[new_position])
        # if new_position in :
    ...
    def attack(self, attacked_piece, price, color):
        ...
        if color == 'black':
            ...
            board.black_killed_pieces.append(attacked_piece)
...
class Rook(Piece):
    ...
    def possible_moves(self):
        ...
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            while self.board.is_within_bounds((nx, ny)):
                if self.board.is_empty((nx, ny)):
                    moves.append((nx, ny))
                elif self.board.is_enemy((nx, ny), self.color):
                    moves.append((nx, ny))
                    break
                else:
                    break
...
class King(Piece):
    ...
    def can_castle(self, kingside=True):
        # Реализация проверки условий рокировки
        pass
